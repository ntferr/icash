...
func TestBankEntities(t *testing.T) {
	t.Run("when you create the correct object then it works", func(t *testing.T) {
		actual := bankSuccess
		err := actual.Validate()
		assert.Nil(t, err)
	...
	t.Run("when code empty then it fails", func(t *testing.T) {
		actual := bankCodeEmpty
		err := actual.Validate()
		assert.Equal(t, err.Error(), "bank code is required")
	})

	t.Run("when code is over 3 characters then it fails", func(t *testing.T) {
